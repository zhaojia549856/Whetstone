...
def classify(entry):
	# print(sum(entry),len(entry), sum(entry)*1.0/len(entry))
	return int(sum(entry)*1.0/len(entry) > 0.5)
...
if len(os.sys.argv) == 2:
	...
	path = "../neuro/apps/class2d/data/v2_testing/"

numClasses = 2
num_problem, num_testing = 1000, 100
...
model = Sequential()
model.add(Conv2D(1, (3, 3), padding='same', activation=None, use_bias=True, input_shape=input_shape))
model.add(BatchNormalization())
model.add(Spiking_BRelu())
model.add(Conv2D(1, (3, 3), padding='same', activation=None, use_bias=True))
model.add(BatchNormalization())
model.add(Spiking_BRelu())
...
# model.add(BatchNormalization())
# model.add(Spiking_BRelu())
# model.add(Conv2D(8, (5, 5), padding='same', activation=None, use_bias=True))
# model.add(BatchNormalization())
# model.add(Spiking_BRelu())
# model.add(MaxPooling2D(pool_size=(2, 2)))
# model.add(Conv2D(16, (3, 3), padding='same', activation=None, use_bias=True))
# model.add(BatchNormalization())
# model.add(Spiking_BRelu())
# model.add(Conv2D(16, (3, 3), padding='same', activation=None, use_bias=True))
# model.add(BatchNormalization())
...
# neuro(new_model, key, "whetstone", path)
# neuro(new_model, key, "danna2", path)
